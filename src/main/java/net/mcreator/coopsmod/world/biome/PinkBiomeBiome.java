...
package net.mcreator.coopsmod.world.biome;
...
import net.mcreator.coopsmod.block.PinkLeafBlock;
import net.mcreator.coopsmod.block.PinkGrassBlock;
...
public class PinkBiomeBiome extends CoopsmodModElements.ModElement {
	@ObjectHolder("coopsmod:pink_biome")
	public static final CustomBiome biome = null;
	public PinkBiomeBiome(CoopsmodModElements instance) {
		super(instance, 29);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.3f).scale(1.2000000000000002f).temperature(0.7f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-1011242).waterFogColor(-1011242)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(PinkGrassBlock.block.getDefaultState(),
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(PinkLogBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(PinkLeafBlock.block.getDefaultState()))).baseHeight(6)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == PinkGrassBlock.block.getDefaultState().getBlock() || ground == Blocks.COARSE_DIRT.getDefaultState().getBlock())
							&& (ground2 == PinkGrassBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == PinkLeafBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, PinkLeafBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, PinkLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == PinkLogBlock.block.getDefaultState().getBlock()
					|| blockType == PinkLeafBlock.block.getDefaultState().getBlock() || blockType == PinkGrassBlock.block.getDefaultState().getBlock()
